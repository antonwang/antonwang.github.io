...
Bird.prototype, which can then be accessed by duck. This is an example of the prototype
chain. In this prototype chain, Bird is the supertype for duck, while duck is the 
subtype. Object is a supertype for both Bird and duck. Object is a supertype for all
...
/* This and the next challenge will cover how to reuse Animal's methods inside Bird 
and Dog without defining them again. It uses a technique called inheritance. This 
challenge covers the first step: make an instance of the supertype (or parent). 
You already know one way to create an instance of Animal using the new operator:  */

let animal = new Animal();
/* There are some disadvantages when using this syntax for inheritance, which are
 too complex for the scope of this challenge. Instead, here's an alternative approach 
...
/* But duck and all instances of Bird should show that they were constructed by Bird
 and not Animal. To do so, you can manually set Bird's constructor property to the 
 Bird object:   */
...
unique to Bird objects. Here, Bird will get a fly() function. Functions are added to
 Bird's prototype the same way as any constructor function:  */
...
flyMixin(bird);
flyMixin(plane);
/* Here bird and plane are passed into flyMixin, which then assigns the fly function
...
let glideMixin = function(obj) {
  obj.glide = function() {
    console.log("It glides");
  }
};

glideMixin(bird);
glideMixin(boat);
bird.glide(); //"It glides"
boat.glide(); //"It glides"


//Object Oriented Programming: Use Closure to Protect Properties Within an Object 
...
the constructor function. This changes the scope of that variable to be within the
constructor function versus available globally. This way, the variable can only be
...
variable hatchedEgg. This is possible because hatchedEgg is declared in the same context
as getHatchedEggCount. In JavaScript, a function always has access to the context in 
...
/*Another example - rewriting the function makeNest() so instead it is an
...
an object motionModule. This returned object contains all of the mixin behaviors as 
