...
<script>
  document.addEventListener('DOMContentLoaded', function(){ //code to execute only once your page has finished loading
    // Add your code below this line
    document.getElementById('getMessage').onclick = function(){}; //implement an onclick event handler on Id
...
<script>
  document.addEventListener('DOMContentLoaded', function(){ //execute only once your page has finished loading
    document.getElementById('getMessage').onclick = function(){ //implement an onclick event handler on Id
      // Add your code below this line
    document.getElementsByClassName('message')[0].textContent="Here is the message"; //upon click, update text of the class name
      // Add your code above this line
    }
  });
</script>
...
<script>
  document.addEventListener('DOMContentLoaded', function(){ //execute only once your page has finished loading
    document.getElementById('getMessage').onclick = function(){ //implement an onclick event handler on Id
      // Add your code below this line
const req = new XMLHttpRequest(); //an instance of the XMLHttpRequest object is created
req.open("GET",'/json/cats.json',true); //open method initializes a request, true = asynchronous request
req.send(); //send method sends the request
req.onload = function(){ //onload event handler parses the returned data and applies the JSON.stringify method to convert the JavaScript object into a string, which is then inserted as the message text
  const json = JSON.parse(req.responseText);
  document.getElementsByClassName('message')[0].innerHTML = JSON.stringify(json);
};
      // Add your code above this line
...
<script>
  document.addEventListener('DOMContentLoaded',function(){ //execute only once your page has finished loading
    document.getElementById('getMessage').onclick= () => { //implement an onclick event handler on Id
      // Add your code below this line
fetch('/json/cats.json') //fetch(URL) makes a GET request to the URL specified. The method returns a Promise
    .then(response => response.json()) //if request was successful, then method is executed. Response --> JSON
    .then(data => { //the argument is JSON object, which JSON stringify converts it to a string...
        document.getElementById('message').innerHTML = JSON.stringify(data);
    })
      // Add your code above this line
    };
...
<script>
  document.addEventListener('DOMContentLoaded', function(){//execute only once your page has finished loading
    document.getElementById('getMessage').onclick = function(){ //implement an onclick event handler on Id
      ...
      req.send(); //send method sends the request
      req.onload=function(){
        const json = JSON.parse(req.responseText); //parses the returned data into a JavaScript object/array
        document.getElementsByClassName('message')[0].innerHTML = JSON.stringify(json);
        // Add your code below this line
      console.log(json[2].codeNames[1]); //accessing array, then object, then array
        // Add your code above this line
      };
    };
  });
...
let html = "";
json.forEach(function(val) { //to loop through data held in an array
  const keys = Object.keys(val); //storing value of keys in const keys
  html += "<div class = 'cat'>"; //adding HTML to the variable
  keys.forEach(function(key) { //loop through data held in keys
    html += "<strong>" + key + "</strong>: " + val[key] + "<br>"; //adding HTML to the variable
  });
  html += "</div><br>";n//adding HTML to the variable
});
  ...
  document.addEventListener('DOMContentLoaded', function(){ //execute only once your page has finished loading
    document.getElementById('getMessage').onclick = function(){ //implement an onclick event handler on Id
      ...
      req.onload = function(){
        ...
        html += "</div><br>"; //adding HTML to the variable
        });
        // Add your code above this line
        document.getElementsByClassName('message')[0].innerHTML = html; //display HTML to class name element
      };
    };
  });
</script>
    ...
    document.getElementById('getMessage').onclick = function(){ //implement an onclick event handler on Id
      ...
      req.onload = function(){
        ...
        json.forEach(function(val) { //to loop through data held in an array
          html += "<div class = 'cat'>"; //adding HTML to the variable
          // Add your code below this line
            html += "<img src = '" + val.imageLink + "' " + "alt='" + val.altText + "'>"; // use imageLink property to display image in an img element.
          // Add your code above this line
          html += "</div><br>"; //adding HTML to the variable
        });
        document.getElementsByClassName('message')[0].innerHTML=html; //display HTML to class name element
...
json = json.filter(function(val) { // use the filter method to filter out
  return (val.id !== 1);
});
...
<script>
  document.addEventListener('DOMContentLoaded', function(){ //execute only once your page has finished loading
    document.getElementById('getMessage').onclick = function(){ //implement an onclick event handler on Id
      ...
      req.send(); //send method sends the request
      req.onload=function(){
        let json = JSON.parse(req.responseText); //parses the returned data into a JavaScript object/array
        let html = "";
        // Add your code below this line
        json = json.filter(function(val) { // creating a new array using the filter method to filter out...
          return (val.id !== 1);
        });
        // Add your code above this line
         json.forEach(function(val) {  //to loop through the filtered data held in an array
           html += "<div class = 'cat'>" //adding HTML to the variable
...
if (navigator.geolocation){ //checks if the navigator.geolocation object exists
...
<script>
  // Add your code below this line
if (navigator.geolocation){ //checks if the navigator.geolocation object exists
  navigator.geolocation.getCurrentPosition(function(position) { //initiates an asynchronous request for the user's position
    document.getElementById('data').innerHTML="latitude: " + position.coords.latitude + "<br>longitude: " + position.coords.longitude;
  });
}
...
xhr.open('POST', url, true); //open method initializes the request as a "POST" to the given URL of the external resource, and uses the true Boolean to make it asynchronous.
xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8'); //sets the value of an HTTP request header
xhr.onreadystatechange = function () { //handles a change in the state of the request
  if (xhr.readyState === 4 && xhr.status === 201){
    //readyState of 4 means the operation is complete, and a status of 201 means it was a successful request
...
<script>
  document.addEventListener('DOMContentLoaded', function(){ //execute only once your page has finished loading
    document.getElementById('sendMessage').onclick = function(){//implement an onclick event handler on Id

      const userName = document.getElementById('name').value; //store userinput in const userName
      const url = 'https://jsonplaceholder.typicode.com/posts';
      
      // Add your code below this line
      const xhr = new XMLHttpRequest(); //an instance of XMLHttpRequest is created
      xhr.open('POST', url, true); //request as a "POST" to the given URL, true = asynchronous request
      xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8'); 
      xhr.onreadystatechange = function () { //handles a change in the state of the request
        if (xhr.readyState === 4 && xhr.status === 201){ //4 = operation is completed, 201 =successful request
          const serverResponse = JSON.parse(xhr.response);
          document.getElementsByClassName('message')[0].textContent = serverResponse.userName + serverResponse.suffix;
        }
      };
      const body = JSON.stringify({ userName: userName, suffix: ' loves cats!' });
      xhr.send(body); //sends the request with the body value, which userName key is the user's input
      // Add your code above this line
