// Contents are taken from freecodecamp's React and Redux tutorials. 
...
class DisplayMessages extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = { //initialize state in the constructor
        input: "",
        messages: []
        };
    }
...
/* hint: You may wanna add the following <div> into render content to check if the handleChange() method is working.
...
class DisplayMessages extends React.Component {
    ...
    handleChange(event) {
        //Updates the input property of the component's state with this new string
        this.setState ({
            input: event.target.value 
        });
    }

    submitMessage() { //concatenate the current message (stored in input) to the messages array in local state
        this.setState ({
            input: '', //clear the value of the input
            messages: [...this.state.messages, this.state.input]
        });
    }
    render() { //items is an array with <li>{item}</li>;
        const items = this.state.messages.map(item => <li>{item}</li>);
        return (
        <div>
            <h2>Type in a new Message:</h2>
            { /* render an input, button, and ul here. Bind the method calls in component attributes or the binding can be done beforehand */ }
...
const addMessage = (message) => { //action creator that adds a message
    return ({
        type: ADD,
        message: message
    });
...
const messageReducer = (state= [], action) => { //addMessage goes into action
    switch(action.type) {
        case ADD:
            return [...state, action.message] //adding message to end of array
        default:
            return state;
    }
}
...
// change code below this line -  to omit one of the arguments to the connect method, pass null in its place
...
class Presentational extends React.Component {
  ...
  submitMessage() {
    //dispatches submitNewMessage from this.props
  ...
  render() {
    return (
      <div>
        ...
        <button onClick={this.submitMessage}>Submit</button> 
        <ul>
          {this.props.messages.map( (message, idx) => { //pulls message info from store
              return (
                 <li key={idx}>{message}</li>
              )
            })
...
const mapStateToProps = (state) => {
  return {messages: state} //pulls message info from store
};

const mapDispatchToProps = (dispatch) => {
  return {
    submitNewMessage: (message) => {
      dispatch(addMessage(message)) //sends message to Redux for processing by store
    }
  }
};

//connecting React to Redux, using mapStateToProps and mapDispatchToProps
const Container = connect(mapStateToProps, mapDispatchToProps)(Presentational); 
